...
test "tui shows details, confirms deletion, and shares annotation semantics" {
    ...
    try child.write("echo DELETE_ME\n");
    try std.testing.expect(try child.readUntilFrom(gpa, &transcript, from, support.test_prompt, support.timeout_ms));

    from = transcript.items.len;
    try child.write("command \"$TJ\" tui\n");
    try std.testing.expect(try child.readUntilFrom(gpa, &transcript, from, "2 entries", support.timeout_ms));

    // The browser starts on the newest completed entry, excluding its own
    // running command. Pin it, decline the first deletion prompt, then
    // explicitly confirm that this pinned entry should be removed.
    from = transcript.items.len;
    try child.write("p");
    try std.testing.expect(try child.readUntilFrom(gpa, &transcript, from, "entry 2 pinned", support.timeout_ms));
    from = transcript.items.len;
    try child.write("d");
    try std.testing.expect(try child.readUntilFrom(gpa, &transcript, from, "is pinned. Delete it anyway?", support.timeout_ms));
    from = transcript.items.len;
    try child.write("n");
    try std.testing.expect(try child.readUntilFrom(gpa, &transcript, from, "delete cancelled", support.timeout_ms));
    from = transcript.items.len;
    try child.write("d");
    try std.testing.expect(try child.readUntilFrom(gpa, &transcript, from, "is pinned. Delete it anyway?", support.timeout_ms));
    from = transcript.items.len;
    try child.write("y");
    try std.testing.expect(try child.readUntilFrom(gpa, &transcript, from, "deleted entry 2", support.timeout_ms));

    from = transcript.items.len;
    try child.write("p");
    try std.testing.expect(try child.readUntilFrom(gpa, &transcript, from, "entry 1 pinned", support.timeout_ms));
    ...
    const tui_out = try journal.read(gpa, "3/out");
