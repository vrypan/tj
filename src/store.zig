...
pub const Store = struct {
    ...
    pub fn begin(self: *Store, cmd: []const u8) void {
        ...
        self.beginFallible(name, cmd) catch |err| {
            self.warn("cannot start interaction {s}: {t}", .{ name, err });
            self.disable();
        };
    }

    fn beginFallible(self: *Store, name: []const u8, cmd: []const u8) !void {
